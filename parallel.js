/**
 * US Visa Appointment Bot - FAST BOOKING (Last Slots First)
 *
 * STRATEGY: Try booking from 11:45 → 07:00 (reverse order)
 * - Most bots/people try 7am first, so later slots have LESS COMPETITION
 * - Fast form submission with proper session handling
 * - Clicks Confirm button to complete booking
 *
 * Features:
 * - Response listener for instant date detection
 * - Cached IDs for speed
 * - Instant booking flag
 * - Stale data verification
 * - Reverse slot order (last first = less competition)
 */
...
// ============================================================================
// ALL TIME SLOTS - Fire ALL 20 at once!
// ============================================================================
        ...
        try {
            ...
            if (url.includes('.json') && url.includes('appointments') && !url.includes('date=')) {
                ...
                if (data && Array.isArray(data) && data.length > 0) {
                    ...
                    // INSTANT TRIGGER: If date in range, set flag immediately
...
// ============================================================================
// BOOKING - TRUE PARALLEL (ALL 20 SLOTS AT ONCE!)
// Fire all POSTs simultaneously → one will set session state → click Confirm
// ============================================================================
async function performBooking(page, slot) {
    const startTime = Date.now();

    // Notify booking started
    sendTelegram(`🚀 <b>FAST BOOKING!</b>\n📅 ${slot.date}\n📍 ${CONFIG.preferences.city}\n⚡ Trying last slots first!`);

    try {
        // Get fresh CSRF token and IDs
        ...
        if (!ids?.scheduleId || !ids?.csrf) {
            log(`❌ Missing IDs or CSRF`, 'ERROR');
            return false;
        }

        cachedIds = ids;
        const appointmentUrl = `${CONFIG.preferences.baseUrl}/schedule/${ids.scheduleId}/appointment`;

        // =====================================================
        // FAST FORM SUBMISSION - Try last slots first (less competition)
        // This is more reliable than parallel fetch()
        // =====================================================
        log(`⚡ FAST BOOKING - trying slots (last first)...`, 'SUCCESS');

        const slotsToTry = [...ALL_TIME_SLOTS].reverse(); // Last slots first

        for (let i = 0; i < slotsToTry.length; i++) {
            const time = slotsToTry[i];
            log(`  ⏳ Trying slot ${i+1}/${slotsToTry.length}: ${time}`, 'INFO');

            try {
                // Make sure we're on the appointment form
                const onForm = await page.$('#appointments_consulate_appointment_date').catch(() => null);
                if (!onForm) {
                    await page.goto(appointmentUrl, { waitUntil: 'domcontentloaded', timeout: 5000 }).catch(() => {});
                    await page.selectOption('#appointments_consulate_appointment_facility_id', ids.facilityId).catch(() => {});
                    await page.waitForTimeout(200);
                }

                // Fill and submit
                const submitted = await page.evaluate(({ date, time }) => {
                ...
                }, { date: slot.date, time }).catch(e => ({ error: e.message }));

                if (submitted.error) {
                    log(`  ❌ ${time}: ${submitted.error}`, 'WARN');
                    continue;
                }

                // Wait for confirm button
                await page.waitForSelector('a.button.alert', { timeout: 2000 });
                await page.click('a.button.alert');

                const elapsed = Date.now() - startTime;
                log(`🎉 BOOKED in ${elapsed}ms! Time: ${time}`, 'SUCCESS');
                sendTelegram(`🎉 <b>BOOKED!</b>\n📅 ${slot.date}\n⏰ ${time}\n⏱ ${elapsed}ms\n📧 ${CONFIG.credentials.email}`);
                return true;

            } catch (slotErr) {
                log(`  ❌ ${time}: ${slotErr.message.substring(0, 50)}`, 'WARN');
                // Always navigate back to form for next attempt
                await page.goto(appointmentUrl, { waitUntil: 'domcontentloaded', timeout: 5000 }).catch(() => {});
                await page.selectOption('#appointments_consulate_appointment_facility_id', ids.facilityId).catch(() => {});
        ...
        log(`❌ All booking attempts failed`, 'ERROR');
...
async function runBot() {
    console.log('\n' + '═'.repeat(60));
    console.log('\x1b[32m  VISA BOT - FAST BOOKING (Last Slots First)\x1b[0m');
    console.log('\x1b[36m  Target: ' + CONFIG.bot.targetCPM + ' CPM\x1b[0m');
    console.log('\x1b[33m  Strategy: 11:45 → 07:00 (less competition on later slots)\x1b[0m');
        ...
        while (true) {
            try {
                ...
                if (checkCount % Math.ceil(CONFIG.bot.targetCPM / 60) === 0) {
                    ...
                    console.log(`\x1b[44m[${cpm} CPM]\x1b[0m #${checkCount} | Lat: ${latencyDisplay} | Slot: ${dateDisplay} | Best: ${closestDisplay}`);
                }
                ...
                if (slot && isDateInRange(slot.date, CONFIG.preferences.startDate, CONFIG.preferences.endDate)) {
                    ...
                    cachedIds = null;                 // Clear cached IDs
                }
