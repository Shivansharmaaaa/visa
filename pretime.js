...
// ============================================================================
// BOOKING - PURE UI (no API, just select from dropdown)
// ============================================================================
async function performBooking(page, slot) {
    ...
    try {
        // Step 1: Set date + wait + select first time - ALL IN ONE evaluate
        log(`📅 Date: ${slot.date}`, 'INFO');
        const selectedTime = await page.evaluate(async (date) => {
            // Set date
            ...
            // Wait 500ms for times to load
            await new Promise(r => setTimeout(r, 500));

            // Select first time option
        ...
        log(`⏰ Time: ${selectedTime || 'NONE'}`, selectedTime ? 'SUCCESS' : 'ERROR');

        if (!selectedTime) {
            log(`❌ No time options`, 'ERROR');
            return false;
        }
        log(`⏰ Time: ${selectedTime}`, 'SUCCESS');

        // Step 4: Click submit
        log(`📤 Submit...`, 'INFO');
        await page.click('#appointments_submit');

        // Step 5: Wait for confirm button
        try {
            await page.waitForSelector('a.button.alert', { timeout: 3000 });
        } catch (e) {
            log(`⚠️ No confirm button`, 'WARN');
        }
        ...
        try {
            ...
            if (e.message.includes('context') || e.message.includes('destroyed')) {
                log(`✅ Confirm navigation`, 'SUCCESS');
            }
        }
