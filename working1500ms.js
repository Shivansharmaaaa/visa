...
const CONFIG = {
    ...
    bot: {
        targetCPM: parseInt(process.env.TARGET_CPM) || 4000,
        headless: process.env.HEADLESS === 'true',
        description: "DIRECT IP MODE - Advanced Stealth Pulse + Hardware Spoofing"
    }
};
...
function getDelay(targetCPM) {
    // Smart Pulse Delay: 4000 CPM is ~15ms per check.
    // We'll vary it between 10ms and 20ms to break pattern recognition.
    const idealCycle = 60000 / targetCPM;
    const jitter = (Math.random() * 10) - 5; // -5ms to +5ms jitter
    return Math.max(0, Math.floor(idealCycle + jitter));
}
        ...
        try {
            ...
            if (url.includes('.json') && url.includes('date=')) {
                ...
                if (data && data.available_times && data.available_times.length > 0) {
                    ...
                    // log(`⏰ Time captured: ${availableTime}`, 'INFO'); // Silenced for high speed
                }
            }
        } catch (e) {
...
// ============================================================================
// WAIT FOR FRESH RESPONSE - OPTIMIZED FOR HIGH SPEED
// ============================================================================
async function waitForAvailableSlot(timeoutMs = 20) {
    ...
    while (lastResponseTime === prevTime && elapsed < timeoutMs) {
        ...
        await new Promise(r => {
            if (typeof setImmediate !== 'undefined') setImmediate(r);
            else setTimeout(r, 0);
        });
        elapsed += 1;
    }
...
async function runBot() {
    ...
    try {
        ...
        log(`Using unique fingerprint: ${fingerprint.navigator.userAgent.substring(0, 50)}...`, 'SECURITY');
        ...
        // Attach the fingerprint (Canvas, WebGL, Audio, etc.)
        await fingerprintInjector.attachFingerprintToPlaywright(context, fingerprintData);
        log('Advanced Fingerprint Injected (Canvas/WebGL/Audio spoofing active)', 'SECURITY');
        ...
        while (true) {
            try {
                ...
                // =====================================================
                // SMART SESSION PULSE - Restart every 5000 checks
                // Prevents long-term IP tracking and clears memory
                // =====================================================
                if (!bookingInProgress && checkCount % 5000 === 0) {
                    log(`🔄 Smart Session Pulse - Restarting for fresh IP footprint...`, 'SECURITY');
                    sendTelegram(`🔄 <b>Smart Session Pulse</b>\nRestarting to avoid IP tracking...`);
                    if (browser) await browser.close().catch(() => {});
                    await new Promise(r => setTimeout(r, 2000)); // 2s pause to look human
                    return runBot();
                }
                ...
                if (!bookingInProgress && checkCount % 500 === 0) {
                    const pageText = await page.innerText('body').catch(() => '');
                    if (pageText.toLowerCase().includes('system is busy')) {
