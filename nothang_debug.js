...
// ============================================================================
// BOOKING - DEBUG VERSION: CAPTURES EXACT REQUESTS
// Does the normal form-submit booking BUT logs every HTTP request
// so we can see exactly what the browser sends
// ============================================================================
async function performBooking(page, slot) {
    ...
    log(`🔍 DEBUG MODE: Capturing all requests during booking`, 'WARN');
    ...
    try {
        ...
        // Fetch times via API (original approach)
        ...
        // =====================================================
        // START REQUEST CAPTURE - intercept ALL outgoing requests
        // =====================================================
        const requestHandler = (request) => {
            const url = request.url();
            // Only capture requests to the visa site
            if (url.includes('usvisa-info.com')) {
                ...
                // Log form submissions and navigations prominently
                if (request.method() !== 'GET' || url.includes('appointment')) {
                    console.log('\n' + '🔍'.repeat(30));
                    console.log(`🔍 CAPTURED ${request.method()} REQUEST:`);
                    console.log(`🔍 URL: ${url}`);
        ...
        const responseHandler = async (response) => {
            const url = response.url();
            if (url.includes('appointment') && url.includes('usvisa-info.com')) {
                console.log('\n' + '📡'.repeat(30));
                console.log(`📡 RESPONSE for: ${url}`);
                console.log(`📡 Status: ${response.status()}`);
                console.log(`📡 Headers: ${JSON.stringify(response.headers(), null, 2)}`);

                // If it's a redirect, log the Location header
                if (response.status() >= 300 && response.status() < 400) {
                    console.log(`📡 REDIRECT TO: ${response.headers()['location']}`);
                }

                // Try to get response body for HTML responses
                try {
                    ...
                    if (contentType.includes('text/html')) {
                        ...
                        const confirmMatch = body.match(/<a[^>]*class="[^"]*button[^"]*alert[^"]*"[^>]*>/gi);
                        if (confirmMatch) {
                            console.log(`📡 CONFIRM BUTTON FOUND: ${confirmMatch[0]}`);
                            // Get the full tag with href
                            const hrefMatch = confirmMatch[0].match(/href="([^"]*)"/);
                            if (hrefMatch) {
                                console.log(`📡 CONFIRM HREF: ${hrefMatch[1]}`);
                            }
                        }

                        // Also search for any links with "confirm"
                        const allConfirmLinks = body.match(/<a[^>]*href="[^"]*"[^>]*>[^<]*[Cc]onfirm[^<]*<\/a>/gi);
                        if (allConfirmLinks) {
                            console.log(`📡 ALL CONFIRM LINKS:`);
                            allConfirmLinks.forEach(link => console.log(`   ${link}`));
                        }

                        // Log first 3000 chars of body (skipping scripts)
                        const bodyContent = body.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '').substring(0, 3000);
                        console.log(`📡 Body (no scripts, first 3000): ${bodyContent}`);
                    }
                } catch (e) {
        ...
        log(`🔍 Request capture active - submitting form now...`, 'WARN');

        // =====================================================
        // ORIGINAL FORM SUBMIT (sets fields + form.submit())
        // =====================================================
        try {
            ...
            if (e.message.includes('context') || e.message.includes('destroyed')) {
                log(`✅ Navigation detected (form submitted)`, 'SUCCESS');
            } else {
                throw e;
            }
        }
        ...
        try {
            await page.waitForSelector('a.button.alert', { timeout: 5000 });
        } catch (e) {
            log(`⚠️ Confirm button wait timeout`, 'WARN');
        }

        // Capture the confirm button's href BEFORE clicking
        try {
            const confirmData = await page.evaluate(() => {
                ...
                return {
                    confirmHref: btn ? btn.href : null,
                    confirmText: btn ? btn.innerText : null,
                    confirmOuterHTML: btn ? btn.outerHTML : null,
                    pageTitle: document.title,
                    pageUrl: window.location.href,
                    allLinks: allLinks.filter(l => l.text.toLowerCase().includes('confirm') || l.classes.includes('alert'))
                };
            });

            console.log('\n' + '⭐'.repeat(30));
            console.log('⭐ CONFIRM PAGE DATA:');
            console.log(`⭐ Page Title: ${confirmData.pageTitle}`);
            console.log(`⭐ Page URL: ${confirmData.pageUrl}`);
            console.log(`⭐ Confirm Button Href: ${confirmData.confirmHref}`);
            console.log(`⭐ Confirm Button Text: ${confirmData.confirmText}`);
            console.log(`⭐ Confirm Button HTML: ${confirmData.confirmOuterHTML}`);
            console.log(`⭐ All Confirm/Alert Links: ${JSON.stringify(confirmData.allLinks, null, 2)}`);
            console.log('⭐'.repeat(30) + '\n');

        } catch (e) {
            log(`Could not capture confirm data: ${e.message}`, 'WARN');
        }

        // Click confirm button (original approach)
        try {
        ...
        // Wait a moment for the final page
        ...
        // =====================================================
        console.log('\n' + '='.repeat(80));
        console.log('📋 ALL CAPTURED REQUESTS SUMMARY:');
        ...
        // Stop after capturing - don't actually claim booking success
        log(`🔍 DEBUG COMPLETE - Check logs above for exact request details`, 'WARN');
        log(`🛑 STOPPING after debug capture`, 'WARN');
        process.exit(0);
        ...
        if (error.message.includes('context') || error.message.includes('destroyed')) {
            // Dump what we have
            console.log('\n' + '='.repeat(80));
            console.log('📋 CAPTURED REQUESTS (before context destroyed):');
            console.log('='.repeat(80));
            capturedRequests.forEach((req, i) => {
                console.log(`\n--- Request #${i + 1} ---`);
                console.log(`URL: ${req.url}`);
            ...
            console.log('='.repeat(80));
            log(`🔍 DEBUG COMPLETE (context destroyed) - Check logs above`, 'WARN');
            process.exit(0);
        }
