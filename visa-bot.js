...
async function performBooking(page, slot) {
    ...
    try {
        ...
        // STEP 1: Click date input IMMEDIATELY
        await page.click('#appointments_consulate_appointment_date');
        await page.waitForSelector('.ui-datepicker', { timeout: 2000 });

        // STEP 2: Navigate to correct month and click day - ALL IN ONE EVALUATE
            ...
            return new Promise((resolve) => {
                const navigateAndSelect = () => {
                    ...
                    if (monthEl.tagName === 'SELECT') {
                        ...
                        currentYear = parseInt(yearEl.value);
                    } else {
                    ...
                    // If correct month, click the day IMMEDIATELY
                    if (currentMonth === targetMonth && currentYear === targetYear) {
                        const cells = document.querySelectorAll('.ui-datepicker td[data-handler="selectDay"] a');
                        for (const cell of cells) {
                            if (parseInt(cell.textContent) === targetDay) {
                                cell.click();
                                resolve(true);
                                return true;
                            }
                ...
                // Fast polling - 20ms intervals
                let attempts = 0;
                const interval = setInterval(() => {
                    if (navigateAndSelect() || attempts++ > 20) {
        ...
        // STEP 3: Wait for time slot (minimal wait - 20ms intervals)
        let timeWait = 0;
        while (availableTime === null && timeWait < 40) {
            await page.waitForTimeout(20);
            timeWait++;
        }

        if (availableTime) {
            // STEP 4: Select time INSTANTLY
            await page.selectOption('#appointments_consulate_appointment_time', availableTime);
            log(`Time: ${availableTime}`, 'SUCCESS');

            // STEP 5: SUBMIT IMMEDIATELY
            await page.click('#appointments_submit');

            // STEP 6: Handle confirmation popup FAST
            try {
        ...
        log('No time slot available', 'WARN');
        return false;
...
async function runBot() {
    ...
    try {
        ...
        const sessionRefreshIntervalMs = 60 * 60 * 1000; // Refresh session every 1 hour
        const cookieResetIntervalMs = 15 * 60 * 1000; // Reset cookies every 15 minutes to prevent queuing
        ...
        while (true) {
            try {
                ...
                // =====================================================
                // SESSION REFRESH - Every 1 hour to prevent stale connections
                // =====================================================
                if (Date.now() - lastSessionRefresh > sessionRefreshIntervalMs) {
                    log('🔄 Hourly session refresh - restarting for fresh connection...', 'INFO');
                    sendTelegram(`🔄 <b>Hourly Refresh</b>\nRestarting for fresh proxy session...`);
                ...
                if (checkCount % Math.ceil(CONFIG.bot.targetCPM / 60) === 0) {
                    console.log(`\x1b[44m[${cpm} CPM]\x1b[0m #${checkCount} | Slot: ${dateDisplay} | Best: ${closestDisplay} | Verify: ${nextVerifyIn}m`);
                }
