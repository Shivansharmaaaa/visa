...
// ============================================================================
// BOOKING - DIRECT API REQUEST (FIXED TIME, NO UI)
// Single POST to form action. Server returns 302 → /instructions = SUCCESS.
// No confirm step needed (debug capture proved there is no confirm button).
// Headers match exactly what the browser sends on form.submit().
// ============================================================================
const FIXED_TIME = '08:15';

async function performBooking(page, slot) {
    const startTime = Date.now();

    sendTelegram(`🚀 <b>BOOKING STARTED (DIRECT API)!</b>\n📅 ${slot.date}\n⏰ ${FIXED_TIME} (fixed)\n📍 ${CONFIG.preferences.city}\n📧 ${CONFIG.credentials.email}`);

    try {
        // Get IDs and CSRF from the page DOM
        ...
        if (!pageData?.scheduleId || !pageData?.facilityId) {
            log(`❌ Missing IDs (schedule: ${pageData?.scheduleId}, facility: ${pageData?.facilityId})`, 'ERROR');
            return false;
        }
        ...
        log(`⏰ FIXED time: ${FIXED_TIME} | 📅 Date: ${slot.date} | 🔗 URL: ${submitUrl}`, 'SUCCESS');

        // =====================================================
        // SINGLE POST - exact same request as form.submit()
        // Captured via debug: POST → 302 → /instructions = BOOKED
        // No confirm step exists.
        // =====================================================
        const bookResult = await Promise.race([
            page.evaluate(async ({ submitUrl, facilityId, csrf, date, time, hiddenFields, origin }) => {
                try {
                    ...
                    // Build POST body - exact same params as browser form.submit()
                    const params = new URLSearchParams();
                    // Hidden fields first (authenticity_token, confirmed_limit_message, etc.)
                    ...
                    const resp = await fetch(submitUrl, {
                        ...
                        headers: {
                            // Exact headers from debug capture:
                    ...
                    // After redirect, check the final URL and page title
                    ...
                    return {
                        ...
                        pageTitle: pageTitle,
                        // SUCCESS = redirected to /instructions page
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout submitting appointment')), 15000))
        ]).catch(e => ({ error: e.message }));

        if (bookResult.error) {
            log(`❌ Booking API error: ${bookResult.error}`, 'ERROR');
            return false;
        }

        log(`📡 Response: status=${bookResult.status}, redirected=${bookResult.redirected}`, 'INFO');
        log(`📡 Final URL: ${bookResult.finalUrl}`, 'INFO');
        log(`📡 Page Title: ${bookResult.pageTitle}`, 'INFO');

        // SUCCESS CHECK: POST → 302 → /appointment/instructions = BOOKED
        if (bookResult.isInstructionsPage) {
            const elapsed = Date.now() - startTime;
            log(`🎉 BOOKED in ${elapsed}ms! Redirected to instructions page.`, 'SUCCESS');
            sendTelegram(`🎉 <b>BOOKED!</b>\n📅 ${slot.date}\n⏰ ${FIXED_TIME}\n⏱ ${elapsed}ms\n📧 ${CONFIG.credentials.email}`);
            return true;
        }

        // NOT BOOKED - log details for debugging
        log(`⚠️ NOT on instructions page. Title: ${bookResult.pageTitle}`, 'WARN');
        log(`📄 Body: ${bookResult.bodySnippet}`, 'INFO');
        return false;

    } catch (error) {
        if (error.message.includes('context') || error.message.includes('destroyed')) {
            const elapsed = Date.now() - startTime;
            log(`🎉 Likely BOOKED in ${elapsed}ms! (context destroyed = navigation)`, 'SUCCESS');
...
async function runBot() {
    console.log('\n' + '═'.repeat(60));
    console.log('\x1b[32m  VISA BOT v2.4 - DIRECT API BOOKING (FIXED TIME)\x1b[0m');
